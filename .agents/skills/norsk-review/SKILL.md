...
The rules you apply are in `AGENTS.md` §4b (read them first). The helper is `tools/review_no.py`; run every Python
...
Do not publish the Codex artifact unless asked.
...
5. Any new pattern worth adding to `AGENTS.md` §4b or to the `LINT` list in `tools/review_no.py`.
