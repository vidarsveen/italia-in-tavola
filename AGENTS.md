...
Keep updates short: what is done, what remains, and the next action. Continue authorised work without
asking the owner to choose routine details. Report concrete progress, not long technical inventories.
Never describe the live site as updated until a deployment has actually completed.
...
- Live site (the real course, full narration, no size limit): https://vidarsveen.github.io/italia-in-tavola/
  ...
  git-ignored.
- Hosted preview (Codex artifact, 16 MB cap, browser voice for most regions): https://Codex.ai/code/artifact/f01358a9-a2b1-40b6-8232-aa3d55343263
...
2. **Text, Norwegian.** `content/<region>.no.js`: `window.READINGS_NO['IT-xx'] = {credits: window.READINGS['IT-xx'].credits, lessons:[…]}`.
   ...
   (instructions in `.Codex/skills/norsk-review/SKILL.md`, helper `tools/review_no.py`: `--report` dumps EN and NO
...
For audio seek/resume tests use `python tools/test/serve.py` on port 8766. It supports HTTP byte ranges;
the simple server can reset playback to zero when a test seeks. Pass the 8766 URL to the test.
