...
2. **Done 2026-09-15** (steps 2 and 3): Toscana 1–3 went through reader + fact reviews, a second pass, Norwegian and
   ...
   *story arc* of each lead as well as its grammatical shape, because two adjacent chapters that both follow a winemaker abroad who
   ...
   apostrophes still fail in this shell, so write ledger text with the Write tool and append the file. **Next: Lazio** (stem `lazio`),
   then Abruzzo. For the record, the original step 2: Toscana 1–3 were drafted in `content/toscana.js` and published to the preview unreviewed. Run the reader
