...
`italia-course.html` after that region's content files.
...
```js
{
  id:'carbonara',          // unique across the whole course, kebab-case; the URL is #/recipes/<id>
  dish:'Carbonara',        // a COURSE['IT-62'].dishes string, character for character, or '' for none
  course:'primo',          // antipasto | primo | secondo | contorno | dolce | base
  ...
  heroCaption:{en:'…', no:'…'},
  wines:['Frascati Superiore'],   // COURSE.wines names, character for character; links to Vinmonopolet
...
3. `notes` explain why, not what — the mechanism a cook can carry to the next dish. Two is usually
   enough.
4. Norwegian is bokmål written to `CLAUDE.md` §4b, from the facts rather than from the English
...
5. Italian names of dishes, pasta shapes, cheeses and cured meats stay in Italian in both languages.
6. Wines must match `COURSE[code].wines` names exactly, or the Vinmonopolet link cannot be built.
...
`italia-course.html`, that each hero exists on disk and is credited, and that no step repeats a
