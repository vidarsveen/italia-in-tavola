...
BOOK = {'en': 'Italia in Tavola', 'no': 'Italia in Tavola'}
SUBTITLE = {'en': 'A course on the regions, wine and food of Italy',
            'no': 'Et kurs om Italias regioner, vin og mat'}


def app_source():
    return open(os.path.join(ROOT, 'italia-course.html'), encoding='utf-8').read()
...
def main():
    ...
    dst = os.path.join(OUT, f'italia-in-tavola-{lang}.m4b')
    build(tracks, dst, lang, gap, bitrate, mp3='--mp3' in a)
    playlist(tracks, os.path.join(OUT, f'italia-in-tavola-{lang}.m3u'))
