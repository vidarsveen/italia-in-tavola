...
"small cryptic messages" the owner complained about. Changing either setting changes the
script, so every file rendered with a different setting is a deliberate re-record; the
settings used are written to manifest.json so `review_no.py --stale` compares like with like.
...
Usage: python tools/narrate.py lazio --intro title --drop facts,recap,tasting,headings --outro none [--only en-3]
